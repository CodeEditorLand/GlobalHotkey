...
struct Example {
	last_pressed:String,
	// store the global manager otherwise it will be dropped and events will
	// not be emitted
...
enum ProgramCommands {
	// message received when the subscription calls back to the main gui
	// thread
	Received(String),
}
...
impl Example {
	pub fn hotkey_sub(&self) -> Subscription<ProgramCommands> {
		iced::subscription::channel(0, 32, |mut sender| {
			async move {
