...
use objc2::{ClassType, msg_send_id, rc::Retained};
		...
		if let Some(scan_code) = key_to_scancode(hotkey.key) {
			let hotkey_id = EventHotKeyID {
				...
				signature:{
					...
					// and confirmed by applying the same method to `kEventParamDragRef` which is
					// equal to `drag` in C and converted to `1685217639` by rust-bindgen.
					for c in "htrs".chars() {
						res = (res << 8) + c as u32;
					}
