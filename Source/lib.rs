...
//!
//! - On Windows a win32 event loop must be running on the thread. It doesn't
//!   need to be the main thread but you have to create the global hotkey
//!   manager on the same thread as the event loop.
//! - On macOS, an event loop must be running on the main thread so you also
//!   need to create the global hotkey manager on the main thread.
//!
//! # Example
//!
//! ```no_run
...
//! [`GlobalHotKeyEvent::receiver`] to get events for the hotkey pressed events.
//! ```no_run
...
/// Describes a global hotkey event emitted when a [`HotKey`] is pressed or
/// released.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
#[cfg_attr(feature = "serde", derive(serde::Deserialize, serde::Serialize))]
...
impl GlobalHotKeyManager {
	pub fn new() -> crate::Result<Self> {
		Ok(Self { platform_impl:platform_impl::GlobalHotKeyManager::new()? })
	}
